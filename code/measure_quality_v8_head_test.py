#!/usr/bin/env python3
"""
Head tests under the v8 prompt.
"""
...
OUTPUT_JSON = OUTPUT_DIR / "quality_assessment_v8_heads.json"
...
# Top heads from critical_heads.json (Layer 3)
HEADS_TO_TEST = [(3, 4), (3, 0), (3, 1), (3, 2), (3, 3)]
...
def get_head_outputs(model, tokens, layer):
    ...
    return cache[f"blocks.{layer}.attn.hook_z"].clone()


def patch_single_head_hook(layer, head, source_attn):
    def hook_fn(attn_output, hook):
        seq_len = min(attn_output.shape[1], source_attn.shape[1])
        attn_output[:, :seq_len, head, :] = source_attn[:, :seq_len, head, :]
        return attn_output
    return hook_fn
...
def assess_head(model, tokenizer, head_cfg: HeadConfig):
    print("\n" + "=" * 70)
    print(f"TESTING HEAD: L{head_cfg.layer}H{head_cfg.head}")
    print("=" * 70)
    ...
    for i, problem in enumerate(TEST_PROBLEMS, 1):
        ...
        source_heads = get_head_outputs(model, baseline_ids, head_cfg.layer)
        hook_name = f"blocks.{head_cfg.layer}.attn.hook_z"
        hook_fn = patch_single_head_hook(head_cfg.layer, head_cfg.head, source_heads)
        ...
        intervened_output = generate_tokens(
            model,
            reasoning_ids,
            max_new_tokens=MAX_NEW_TOKENS,
            do_sample=DECODING["do_sample"],
    ...
    return {
        "head": {"layer": head_cfg.layer, "head": head_cfg.head},
        "summary": summary,
...
def main():
    print("\n" + "=" * 70)
    print("V8 HEAD-LEVEL PATCH TEST")
    print("=" * 70)
    ...
    for layer, head in HEADS_TO_TEST:
        result = assess_head(model, tokenizer, HeadConfig(layer, head))
